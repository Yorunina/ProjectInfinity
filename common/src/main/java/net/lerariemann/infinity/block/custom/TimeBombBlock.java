        ...
        if (state.get(ACTIVE)) {
            ...
            if (player.isSneaking()) {
                if (InfinityMethods.isInfinity(world)) {
                    if (world instanceof ServerWorld w && !InfinityMethods.isTimebombed(w)) {
                        eraseWorld(w);
                        Events.postTimeBombResetDim(world, state);
                        ...
                        world.playSound(null, pos, ModSounds.IVORY_MUSIC_CHALLENGER_EVENT, SoundCategory.RECORDS, 0.5f, 1f);
                    }
                    return ActionResult.SUCCESS;
                } else if (world instanceof ServerWorld) {
                    world.playSound(null, pos, ModSounds.BACKPORT_VAULT_EVENT, SoundCategory.BLOCKS, 0.5f, 1f);
